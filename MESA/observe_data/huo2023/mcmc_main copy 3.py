...
def interp_data(x, y, x0):
    # 判断是否所有的x都相同
    if np.all(x == x[0]):
        return y.tolist()  # 如果所有x相同，返回所有对应的y值
    
    # 插值
    interp_func = interp1d(x, y, kind='cubic')  # 'cubic'表示三次样条插值

    # 寻找交点
    intersection_points = []
    for x_guess in x:
        try:
            intersection_point = fsolve(lambda x: interp_func(x) - x0, x_guess, xtol=1e-8)[0]
...
def log_likelihood(p0, observed_log_Teff, observed_log_Teff_err, observed_log_g, observed_log_g_err, observed_log_he, observed_log_he_err):
    ...
    for i in range(len(files)):
        ...
        for component in components:
            ...
            predicted_log_teff = data[i]['log_Teff']
            predicted_log_g = data[i]['log_g']
            predicted_log_he = np.log10(Y_surf/4/(1-Y_surf))
            chi2_0=[]
            for n in range(len(data[i])):
                chi2_0.append(chi2_sol(observed_log_Teff, predicted_log_teff[n], observed_log_Teff_err) + chi2_sol(observed_log_g, predicted_log_g[n], observed_log_g_err) + chi2_sol(observed_log_he, predicted_log_he, observed_log_he_err))
            chi2_1=min(chi2_0)
            chi2_component.append(0)  # 初始化每个元素
            
            chi2_component[-1] += chi2_1
